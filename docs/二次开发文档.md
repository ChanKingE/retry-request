...
```mermaid
flowchart TB
    subgraph REGISTER["插件注册"]
        direction TB
        V["use / once 安装插件<br/>注册拦截器、中间件与解析器"]
    end

    subgraph PREPARE["请求准备"]
        direction TB
        A["业务代码 / Service 层"] --> B["合并默认配置"]
        B --> C["请求拦截器链"]
        C --> D["解析 baseURL 与 URL"]
    end

    subgraph EXECUTE["请求执行"]
        direction TB
        E["请求中间件链"] --> F["执行重试策略"]
        F --> G["按顺序执行请求解析器"]
        G --> H{"解析器是否命中"}
    ...
    subgraph RESPONSE["响应处理"]
        direction TB
        W["形成成功或失败的响应 Promise"] --> M["响应拦截器链"]
        M --> O{"拦截器链最终结果"}
        O -->|成功或已恢复| P["读取 response.data"]
        O -->|未恢复的错误| Q["最终错误标准化"]
    end

    subgraph FINISH["请求结束"]
        direction TB
        P --> R["清理本次 once 插件（如有）"]
        Q --> S["清理本次 once 插件（如有）"]
        R --> T["返回业务数据"]
...
export interface RequestOptions<TBody = Record<string, unknown>> extends CommonOptions {
  url: string;
...
业务侧需要增加单次请求字段时，不要直接改库源码里的 `RequestOptions`。通过声明合并扩展
`RequestOptionsExtensions`，扩展后的字段会进入快捷方法 config、请求拦截器、插件和适配器
接收到的最终 `config`：
...
export class RequestClient {
  private requestInterceptors = new InterceptorManager<RequestOptions>();
  ...
  useRequestInterceptor(interceptor: Interceptor<RequestOptions>) {
  ...
  get<T, P = unknown>(url: string, config?: Omit<RequestOptions<P>, "url" | "method">) {
...
> **注意**：请求拦截器收到的 `data`、`params`、`headers` 默认为对象。上面的 `request`
> 省略了其他默认配置、重试、取消、错误转换等逻辑，完整实现见下文。
...
export function createRequest(client: RequestClient) {
  ...
  const execute = <T>(config: RequestOptions) =>
...
内置插件配置应遵循同一优先级：单次请求 `config.[插件名]` >
插件初始化参数 > 插件默认值。当前约定 `config.dedupe` 覆盖去重窗口和去重键，
`config.mock` 覆盖 Mock 响应或 MockRoute，`config.logger` 覆盖当前请求使用的日志实现；
去重默认只合并同时进行的 GET/HEAD 请求，成功响应窗口复用需显式开启 `cacheSettled`。
