...
> **提示**：如果后端返回统一包装格式 `{ code, data, message }`，创建客户端时需要将第二参数
> `responseEnvelope` 设为 `true` 或传入字段映射。启用解包后，泛型参数填写 `data` 的实际类型。
...
```typescript
import { createHttpClient, NetworkError, TimeoutError, HttpError, BusinessError } from "@/request";
...
`RetryPolicy` 可设置 `maxDelay` 限制单次等待，使用 `jitter: "full"` 分散重试时间，
或使用 `respectRetryAfter: true` 遵守 HTTP 响应的 `Retry-After`；这些选项默认不启用。

```typescript
// 简单重试 3 次
await httpClient.get<Data>("/api/data", { retry: 3 });

// 精细控制
await httpClient.get<Data>("/api/data", {
  retry: {
    max: 3,
    delay: 500,
...
如果 `fulfilled` 抛出错误或返回 rejected Promise，该错误会交给同一个拦截器的第二个
`rejected` 函数；`rejected` 返回响应即可恢复后续链路。
...
httpClient
  .get<User>("/api/user", { signal: controller.signal })
  .then(setUser)
  .catch((err) => {
...
useEffect(() => {
  ...
  httpClient.get<User>("/api/user", { signal: controller.signal }).then(setUser);
...
```typescript
get<T, P>(url, config?: Omit<RequestOptions<P>, "url" | "method">): Promise<T>
```

GET 请求的查询参数通过 `config.params` 传入，并由适配器序列化为查询字符串。
...
```typescript
createHttpClient(options?: ClientOptions, responseEnvelope?: boolean | ResponseEnvelopeOptions): RequestClient
```

创建新的 RequestClient 实例。常用选项：

第二参数 `responseEnvelope` 默认为 `false`；传入 `true` 启用标准业务响应解包，或传入
`ResponseEnvelopeOptions` 自定义成功码和字段映射。
...
其中 `baseURL`、`timeout`、`retry`、`withCredentials`、`headers` 是客户端与单次请求
共用的 [`CommonOptions`](types.ts) 字段，单次请求配置可覆盖这些默认值。

请求拦截器收到的 `data`、`params`、`headers` 默认为空对象；默认请求头和单次请求头按字段合并。
Fetch 与 Axios 适配器不会向 GET/HEAD 发送请求体。

内置插件可直接从单次请求配置读取同名字段，并优先于插件初始化参数：
`config.dedupe` 覆盖请求去重插件配置，`config.mock` 覆盖 Mock 响应或路由，`config.logger`
覆盖日志插件配置。去重默认只合并同时进行的 GET/HEAD；`cacheSettled: true` 可在 `windowMs`
窗口内继续复用成功响应。
...
默认客户端返回原始响应数据。创建客户端时传入第二参数 `true`，或注册
`createResponseEnvelopeInterceptor()` 后，才会按 `{ code, data, message }` 解包；此时业务层类型
应填写 `data` 字段的实际类型。
