...
其中 `baseURL`、`timeout`、`retry`、`withCredentials`、`headers`、`meta` 是客户端与单次请求
共用的 [`CommonOptions`](types.ts) 字段，单次请求配置可覆盖这些默认值。

全局 `meta` 会先于单次请求 `meta` 合并，同名键由单次请求覆盖；合并结果会传给请求拦截器、
请求解析器、适配器和响应拦截器。

内置插件可直接从单次请求配置读取同名字段，并优先于 `meta` 和插件初始化参数：
`config.dedupe` 覆盖请求去重插件配置，`config.mock` 覆盖 Mock 响应或路由，`config.logger`
覆盖日志插件配置；旧的 `meta.dedupe`、`meta.mock` 和 `meta.logger` 写法仍然兼容。
