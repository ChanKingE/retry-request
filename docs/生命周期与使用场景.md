...
### `createHttpClient(options?, responseEnvelope?)`

**运行时机：** 应用初始化、模块初始化或创建独立请求实例时调用；不会随每次请求重复执行。

**使用场景：**

- 配置共享的 `baseURL`、timeout、headers 和 retry。
- 选择 `FetchAdapter`、`AxiosAdapter`、`UniAppAdapter` 或自定义 Adapter。
- 按需启用业务响应解包。

```ts
import { createHttpClient } from "@chan98/request";
...
默认客户端使用 `FetchAdapter`、10 秒超时，不解包业务响应。第二参数传入 `true` 可启用
`{ code, data, message }` 解包，也可传入 `ResponseEnvelopeOptions` 自定义字段映射。
...
```ts
import { createDedupePlugin } from "@chan98/request";
...
内置插件的初始化参数可被单次请求配置覆盖，优先级为
`config.[插件名]` > `createXxxPlugin(options)` > 插件默认值。
例如 `config.dedupe` 覆盖请求去重配置，`config.logger` 覆盖日志输出配置，`config.mock`
覆盖 Mock 响应或路由。去重默认只合并同时进行的 GET/HEAD 请求。
...
```ts
import { createRequest } from "@chan98/request";
...
**输入和输出：** `fulfilled` 接收 `RequestOptions` 并返回新的 `RequestOptions`；`rejected` 可以恢复
前面请求拦截器产生的错误。
...
**输入和输出：** 接收最终 `RequestOptions` 与 `next()`。调用 `next()` 会继续后续请求过程；直接
...
**输入和输出：** 接收最终 `RequestOptions`；返回 `HttpResponse` 表示已处理请求，返回 `undefined`
...
也可以将两个函数直接作为参数传入：`useResponseInterceptor(fulfilled, rejected)`。如果当前
`fulfilled` 抛出错误或返回 rejected Promise，该错误会先交给当前拦截器声明的 `rejected`；
如果 `rejected` 未处理，则继续沿拦截器链向后传播。
...
`createHttpClient()` 默认不注册它；第二参数设为 `true` 或传入字段映射时会注册。
...
```ts
import { createResponseEnvelopeInterceptor } from "@chan98/request";
...
import { AxiosAdapter, createHttpClient } from "@chan98/request";
...
```ts
import { isAbortError, isRequestError } from "@chan98/request";
